      ...
      sidebarLayout(
        ...
        selectInput(
          ...
          choices = list("Art" = "Art", "Comics" = "Comics",
                         "Crafts" = "Crafts", "Dance" = "Dance",
                         "Design" = "Design", "Fashion" = "Fashion",
                         "Film & Video" = "Film & Video", "Food" = "Food",
                         "Games" = "Games", "Journalism" = "Journalism",
        ...
        mainPanel(
          ...
          tags$p(class = "succ_summary top", "This chart gives a breakdown of
                 the categories of Kickstarter projects by number of projects,
                 or the breakdown of subcategories for a specific category
                 chosen on the left. You may use this chart to help determine 
                 the most popular categories and subcategories are, which may
                 help inform which is most popular or which would have the most
                 competition.")
        )
      )
    ),
        ...
        sidebarPanel(
          ...
          selectInput("category", label = h3("Select Category"),
                      choices = list("Film & Video" = "Film & Video",
                                     ...
                                     "Dance" = "Dance"),
                      selected = "Film & Video"),
          sliderInput("goal_range", label = h3("Goal Range"), min = 0,
                      max = 1000000, value = c(0, 500000), step = 5000),
          sliderInput("pledged_range", label = h3("Pledged Range"), min = 0,
                      max = 1000000, value = c(0, 500000), step = 5000)
        ),
        mainPanel(
          plotOutput("plot_money", height = 500),
          tags$p(class = "succ_summary", "This plot shows trends between the
                 target fundraising amount of the project in comparison to the
                 actual amount raised. The plots can be adjusted based on the
                 min and max amounts for either axis in order to see trends for
                 projects in a similar price range to your potential project.
                 The projects that lie along the bottom of the graph are likely
      ...
      fluidRow(
        column(10, offset = 1,
          ...
          tags$p(class = "succ_summary", "Here is a graph about the Backers, by
                 ...
                 Whatever the cause may be, as a Backer it would be wise to
                 look at that category as an area in which one can incur more
    ...
    tabPanel(
      ...
      sidebarLayout(
        ...
        mainPanel(
          ...
          p("This interactive graph depicts how far each Kickstarter project has
            reached toward its goal. We can filter by a specific goal range
            (i.e. how much money the Kickstarter project was looking for) as
            well as the launch day of the week. Surprisingly, the distributions
            for each metric is roughly the same! In this case, Kickstarter
            projects have roughly the same chance of success or failure
            regardless of how much money the project aims to crowd-source or
            when the project begins."), br(), br()
        )
      )
    )
  )
